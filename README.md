...
Ставится на отдельный ПК с GPU между телефоном и реальным приёмником
(XRemotexServer). Перехватывает команду начала воспроизведения, прогоняет
поток через нейросетевой/шейдерный апскейлер в реальном времени и отдаёт
приёмнику уже улучшенную картинку вместо оригинала — при этом ни само
приложение AniLabX, ни XRemoteDesktop не модифицируются и не знают о
подмене.

## Зачем это нужно

AniLabX кастует видео на ТВ/ПК-приёмник поверх собственного протокола:
телефон находит приёмник в локальной сети (mDNS) и посылает ему прямую
ссылку на исходный поток. Приёмник сам качает и проигрывает этот поток через
mpv — никакой обработки на лету там нет. Если исходник низкого разрешения
(720p и ниже, что для многих озвучек аниме — норма), на большом экране это
выглядит соответствующе.

XUpscaleNode встраивается прозрачно: телефон вместо реального приёмника
...
```mermaid
flowchart LR
    subgraph phone["Телефон (AniLabX)"]
        A[Приложение]
    end
    ...
    subgraph recv["Реальный приёмник (XRemotexServer)"]
        E[mpv]
    end
...
Узел не заменяет приёмник, а встаёт между телефоном и им:

1. **Обнаружение.** Узел поднимает собственный mDNS-анонс того же типа
   сервиса (`_xremote._tcp`), что и настоящий приёмник, с именем
   `<имя> [Upscale]`. В приложении он появляется вторым пунктом в списке
   целей для трансляции, рядом с настоящим приёмником.
2. **Перехват.** Телефон посылает узлу ту же команду `PLAY_VIDEO`, что
   отправил бы настоящему приёмнику — с прямой ссылкой на исходный HLS-поток
...
4. **Подмена и форвард.** В команде `PLAY_VIDEO` узел подменяет ссылку на
...
5. **Обратный проброс.** Всё остальное (пауза, громкость, перемотка,
   ...
   между телефоном и приёмником в обе стороны, при необходимости поправляя
   позицию воспроизведения под правильный масштаб.
...
этого узел сначала быстро копирует (`-c copy`, без перекодирования) кусок
оригинала от 0 до точки резюма, и уже на нём наращивает апскейленный хвост:
...
метками времени — `seekTo()` на нужную позицию отрабатывает штатно,
...
той же локальной сети, что и телефон с приложением, и настоящий приёмник.
...
2. Запустить `build.bat` — он поставит Python-зависимости во временный
   ...
   в корне проекта.
3. Запустить `XUpscaleNode.exe` (или `start.bat`) — откроется окно
   настроек.
4. Указать адрес и порт настоящего XRemotexServer, при необходимости
   поправить остальные параметры, нажать кнопку запуска.
5. В приложении AniLabX на телефоне в списке целей для каста появится
   второй пункт `<имя> [Upscale]` — выбрать его вместо обычного приёмника.

Готовую собранную папку (`XUpscaleNode.exe` + `_internal/`) после этого
можно копировать на другие ПК как есть — Python там ставить не нужно,
...
- **Приёмник** — адрес и порт настоящего XRemotexServer.
...
- Обнаружение и протокол реализованы по образцу настоящего
  `XRemoteDesktop` — сам он не модифицируется и не требует установки на
  узел.
