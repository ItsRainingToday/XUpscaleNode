...
- **приложение** — AniLabX (Android-версия, с включённым XRemote) —
...
2. **Перехват.** Приложение посылает узлу ту же команду `PLAY_VIDEO`, что
   отправило бы настоящему приёмнику — с прямой ссылкой на исходный HLS-поток
...
5. **Обратный проброс.** Всё остальное (пауза, громкость, перемотка,
   ...
   между приложением и приёмником в обе стороны, при необходимости
...
3. Запустить `XUpscaleNode.exe` (или `start.bat`) — откроется окно
   настроек.
4. Указать адрес и порт настоящего приёмника, при необходимости поправить
   остальные параметры, нажать кнопку запуска.
5. В приложении AniLabX в списке целей для каста появится второй пункт
   `<имя> [Upscale]` — выбрать его вместо обычного приёмника.
